...
def img_from_db(numImgs,final_path="", path="./", stitch = False):
    ...
    global this_path
    this_path = path + "\\src\\dataset\\" if final_path!="" else path #if final path is local then this path is in dataset
...
#         print("ImgCount is ",imgCount)
    ...
    for i in range(imgCount,imgCount+numImgs):# downloading from 500
        ...
        gmaps = googlemaps.Client('AIzaSyDfqR9EWkkk-4qMG21VJbU-kA5n6yhBu0E')
        ...
        if stitch:
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                params = [{
                            ...
                            'heading': str(heading - 30),
                            'pitch': '0',
                            #'fov': '100',
                            'source': 'outdoor',
                            'key': 'AIzaSyDfqR9EWkkk-4qMG21VJbU-kA5n6yhBu0E'
                        }]
                print(params)
                results = google_streetview.api.results(params)
                print(results.metadata)
                ...
                if id in imgs_seen:
                    print("id already exists... going to next")
                    locFile.write(f"{c[0]} {c[1]}, {r[0]} {r[1]}, {heading}, 2\n")
                    continue
                imgs_seen.add(id)

                image = Image.open(tmpdirname + '/gsv_0.jpg')
                image.save(this_path + 'Unlabel/gsv_' + str(3*i) + '.jpg')
                image.close()
                
# commented below allows to download 6 images per location, 3 for heading and 2 for pitch. Can change the steps1 and steps2 to change the
# number of pictures per location. When using this part, please comment the upper part(downloading only one part).
...
#                             'size': '600x600',  # max 640x640 pixels
#                             'location': str(r[0]) + ',' + str(r[1]), #note json format returns long,lat backwards
#                             'heading': str(heading - 30 + j*90/steps1),
#                             'pitch': str(0 + k*30/steps2),
#                             #'fov': '100',
#                             'source': 'outdoor',
#                             'key': 'AIzaSyDfqR9EWkkk-4qMG21VJbU-kA5n6yhBu0E'
#                         }]
...
#                         image = Image.open(tmpdirname + '/gsv_0.jpg')
#                         image.save(this_path + 'Unlabel/gsv_' + str(6*i+j*2+k) + '.jpg')
#                         image.close()
